...
- **SELECT_REQUEST** — инициируется из iframe, когда React-приложению нужно выбрать произвольный элемент. Payload сообщения:
  ...
  ```
  Эти данные передаются как query-параметры `IBLOCK_ID`, `IBLOCK_TYPE` и `lang` к стандартному popup. В качестве `func_name`/`n` указывается временная JS-функция, которую создаёт `install/assets/js/integration.js`; выбранный ID элемента передаётся в эту функцию, после чего диалог закрывается.
- **ADD_OFFER_REQUEST** — вызывается при добавлении торгового предложения. Для построения ссылки на popup используются данные INIT: `initData.iblocks.offers` (ID инфоблока предложений), `initData.iblocksTypes[<ID>]` (код его типа) и `initData.lang` (язык). Структура `initData` формируется AJAX-запросом к `calculator_ajax.php` и описана в разделе «AJAX API». Если пользователь закрывает окно без выбора, Bitrix отправляет в iframe сообщение `ADD_OFFER_CANCELLED`.

Во всех случаях открывается стандартный интерфейс поиска/выбора элемента инфоблока с возможностью фильтрации, а успешный выбор возвращает только ID выбранного элемента, который затем используется для загрузки данных через `REFRESH_RESULT`.
